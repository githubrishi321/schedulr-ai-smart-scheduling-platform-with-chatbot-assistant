...
export default function EditEventTypePage() {
    ...
    async function loadData() {
      try {
        ...
        reset({
          ...
          duration: String(et.duration || 30),
          location: et.location || '',
          buffer_before: String(et.buffer_before || 0),
          buffer_after: String(et.buffer_after || 0),
          color: et.color || '#6366F1',
        });
        
        setSelectedColor(et.color || '#6366F1');
        setQuestions(et.questions || []);
      } catch (err) {
        toast.error('Event type not found or failed to load');
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(e.message || 'Failed to update event type');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (pageLoading) {
    return <div className="animate-pulse space-y-4 max-w-2xl mx-auto"><div className="h-8 bg-[#1A1A2E] rounded w-1/3"></div><div className="h-64 bg-[#1A1A2E] rounded"></div></div>;
  }
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold font-['Syne']">Edit Event Type</h1>
          <p className="text-[#8888AA] text-sm">Update your booking options</p>
        </div>
      </div>
